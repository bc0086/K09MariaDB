...
# 게시판 테이블(자식)
/*
	AUTO_INCREMENT : 오라클의 시퀀스를 대체할 수 있는 속성으로 지정된 컬럼은
		자동으로 값이 증가하게 된다. 단 자동증가로 지정되면  데이터를 임의로
		삽입할 수 없게 된다.
*/
ts_testcreate table board
(
...
/* 자동증가 컬럼에 임의의 값을 삽입할 수 있으나 사용하지 않는것이 좋다. */
...
/*
	날짜타입의 컬럼에 현재날짜를 입력할때 오라클은 sysdate를 사용하지만
	MySQL은 now() 함수를 사용한다.
*/
...
/*
	페이지처리를 위해 게시물의 범위를 정할때 
	Oracle 은 rownum의 속성을 사용하지만 MariaDB는 limit를 사용한다.
	방법 : limit 시작인덱스, 가져올레코드 갯수
*/
-- 1페이지 레코드 셋
